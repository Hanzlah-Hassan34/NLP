...
You can directly help patients book appointments, suggest available appointment slots, confirm bookings,
check available times from the scheduling tool context, and guide rescheduling or cancellation.
Do not tell the patient to use a website or another system to book if the booking can be handled here.
Do not say you cannot check appointment times when [AVAILABLE SLOTS], [APPOINTMENT FOUND], or
[BOOKING CONFIRMED] tool context is present. Use the provided tool context confidently.
...
Booking Methods:
    Online by website, by call on Hospital Number, or by the chat with me
SERVICES OFFERED:
...
TONE & STYLE:
  ...
  - If available slots are present, ask the patient to choose one of those exact slots.
...
HARD LIMITS - NEVER violate these:
  ...
  - For emergencies (severe pain, swelling, bleeding), offer the soonest slot AND give hotline: 042-35001234.
  - Never finalize a booking until the patient has selected a specific offered slot and then confirmed it.""".strip()
...
If [AVAILABLE SLOTS] tool context is present, use those exact slots and do not invent different times.
...
Do NOT book anything yet - offer options and wait for the patient to choose one specific slot.
After showing slots, explicitly ask: which slot would you like me to book?
...
Do not say you cannot book the appointment. You can confirm it here once they approve.""",
    "BOOKING_DONE": """
...
Then offer 2-3 alternative slots from tool context when available. Confirm the new slot before finalizing.
Do not tell the patient to use a website or another system.""",
    "CANCEL": """
...
and offer to rebook in the future. Do not tell the patient to use a website or another system.""",
    "FAQ": """
...
def normalize_time(text: str) -> Optional[str]:
    match = re.search(r"\b(\d{1,2})(?::(\d{2}))?\s?(am|pm)\b", text.strip(), re.IGNORECASE)
    if not match:
        return None
...
class ToolOrchestrator:
    ...
    def book_confirmed_appointment(
        ...
        if not (slots.patient_name and slots.contact_number and
            ...
            print(
                "[TOOLS] Booking skipped due to missing fields: "
                f"name={slots.patient_name}, contact={slots.contact_number}, "
                f"service={slots.service_type}, date={slots.preferred_date}, "
                f"time={slots.preferred_time}"
            )
            return ""
        print(
            "[TOOLS] Booking appointment with slots: "
            f"name={slots.patient_name}, service={slots.service_type}, "
            f"date={slots.preferred_date}, time={slots.preferred_time}"
        )
    ...
    def find_existing_appointment_context(self, slots: AppointmentSlots, purpose: str) -> str:
        """Look up an existing appointment by patient name and optional date."""
        if not slots.patient_name:
            return ""

        appointments = self.appointment.get_patient_appointments(slots.patient_name)
        if not appointments:
            return (
                f"[APPOINTMENT NOT FOUND] No confirmed appointments were found for {slots.patient_name}. "
                f"Ask the patient to verify their name and appointment date for {purpose}."
            )

        selected = None
        if slots.preferred_date:
            for appt in appointments:
                if appt.get("date") == slots.preferred_date:
                    selected = appt
                    break
        if selected is None:
            selected = appointments[0]

        slots.appointment_id = selected["id"]
        slots.current_appointment_date = selected["date"]
        return (
            f"[APPOINTMENT FOUND] Appointment #{selected['id']} for {selected['patient_name']} "
            f"is on {selected['date']} at {selected['time']} with {selected['doctor']} "
            f"for {selected['service']}. Use this for {purpose}."
        )

    def cancel_confirmed_appointment(self, slots: AppointmentSlots) -> str:
        if not slots.appointment_id:
            return "[CANCELLATION FAILED] No appointment was selected to cancel."
        result = self.appointment.cancel_appointment(slots.appointment_id)
        if result.get("success"):
            return f"[CANCELLATION CONFIRMED] {result['message']}"
        return f"[CANCELLATION FAILED] {result.get('message', 'Unknown error')}"
    ...
    def dispatch(
        ...
        if new_state == State.BOOKING_DATETIME or prev_state == State.BOOKING_DATETIME:
            print("[TOOLS] Appointment slot lookup used")
            triggered_tools.append("appointment_slots")
        ...
        if new_state == State.BOOKING_DONE:
            ...
            if booking_ctx:
                print("[TOOLS] Appointment booking used")
                triggered_tools.append("appointment_booking")
                context_parts.append(booking_ctx)

        if new_state in {State.RESCHEDULE, State.CANCEL} or prev_state in {State.RESCHEDULE, State.CANCEL}:
            purpose = "rescheduling" if new_state == State.RESCHEDULE or prev_state == State.RESCHEDULE else "cancellation"
            existing_ctx = self.find_existing_appointment_context(slots, purpose)
            if existing_ctx:
                print("[TOOLS] Appointment lookup used")
                triggered_tools.append("appointment_lookup")
                context_parts.append(existing_ctx)
            if new_state == State.RESCHEDULE or prev_state == State.RESCHEDULE:
                print("[TOOLS] Appointment slot lookup used")
                triggered_tools.append("appointment_slots")
                context_parts.append(self.get_open_slots_context())
            if prev_state == State.CANCEL and intent == "affirm":
                cancel_ctx = self.cancel_confirmed_appointment(slots)
                print("[TOOLS] Appointment cancellation used")
                triggered_tools.append("appointment_cancel")
                context_parts.append(cancel_ctx)
        ...
        if any(kw in lower for kw in cost_keywords):
            print("[TOOLS] Dental cost lookup used")
            triggered_tools.append("dental_cost")
        ...
        if any(kw in lower for kw in weather_keywords):
            print("[TOOLS] Weather lookup used")
...
class LocalLLMEngine:
    ...
    def generate(self, messages: List[Dict[str, str]], max_tokens: int = 150) -> Dict[str, object]:
        if self.llm is None:
        ...
        if messages:
            messages[0]["content"] = (
                ...
                "IMPORTANT: Return only the final patient-facing reply. "
                "Do not include reasoning, chain-of-thought, analysis, or <think> tags."
            )
        prompt_tokens = self.count_tokens(self._messages_to_text(messages))
        safety_margin = 32
        available_completion_tokens = self.n_ctx - prompt_tokens - safety_margin
        if available_completion_tokens <= 0:
            raise ValueError(
                f"Prompt too large ({prompt_tokens} tokens) for context window {self.n_ctx}"
            )
        max_tokens = min(max_tokens, available_completion_tokens)
        ...
        print(f"[LLM ENGINE] Response received with usage={usage}")
        return {
            "content": self._strip_reasoning(resp["choices"][0]["message"]["content"]),
        ...
        if state == State.BOOKING_DATETIME:
            ...
            if parsed:
                ...
                if not self.slots.preferred_date:
                    ...
                    print(f"[SLOTS] Captured preferred_date={self.slots.preferred_date}")
            normalized_time = normalize_time(text)
            if normalized_time and not self.slots.preferred_time:
                self.slots.preferred_time = normalized_time
                print(f"[SLOTS] Captured preferred_time={self.slots.preferred_time}")
            return
...
class ConversationManager:
    ...
    @staticmethod
    def _message_token_count(messages: List[Dict[str, str]], counter: Callable[[str], int]) -> int:
        return sum(counter(message.get("content", "")) for message in messages)

    def _fit_messages_to_context(
        self,
        messages: List[Dict[str, str]],
        reserved_completion_tokens: int = 150,
        safety_margin: int = 32,
    ) -> List[Dict[str, str]]:
        fitted = [dict(message) for message in messages]
        limit = max(self.llm_engine.n_ctx - reserved_completion_tokens - safety_margin, 1)

        while len(fitted) > 2 and self._message_token_count(fitted, self.llm_engine.count_tokens) > limit:
            fitted.pop(1)
    ...
    @staticmethod
    def _looks_like_internal_reasoning(response: str) -> bool:
    ...
    def chat(self, user_input: str) -> tuple[str, State]:
        ...
        prompt_tokens = self.llm_engine.count_tokens(
            self.llm_engine._messages_to_text(messages)
        )
        if prompt_tokens > max(self.llm_engine.n_ctx - 182, 1):
        ...
        if self.llm_engine.enabled:
            ...
            try:
                ...
                print(f"[RESPONSE] Context overflow avoided: {exc}")
                reply = self._fallback_reply(user_input, intent, tool_context)
            if not reply or self._looks_like_internal_reasoning(reply):
                print("[RESPONSE] Detected reasoning-style output, using fallback reply")
                reply = self._fallback_reply(user_input, intent, tool_context)
        else:
    ...
    def _fallback_reply(self, user_input: str, intent: str, tool_context: str = "") -> str:
        s = self.memory.slots
        lower = user_input.lower()

        if "[BOOKING CONFIRMED]" in tool_context:
            appt_id_match = re.search(r"Appointment ID: #(\d+)", tool_context)
            appt_id_text = f" Your appointment ID is #{appt_id_match.group(1)}." if appt_id_match else ""
            return (
                f"Your appointment is confirmed for {s.preferred_date or 'the selected date'} "
                f"at {s.preferred_time or 'the selected time'}.{appt_id_text} "
                "You will receive a reminder soon. Is there anything else I can help with?"
            )

        if "[CANCELLATION CONFIRMED]" in tool_context:
            cancellation_info = tool_context.replace("[CANCELLATION CONFIRMED]", "").strip()
            return f"{cancellation_info} If you'd like, I can also help you book a new appointment."

        if "[APPOINTMENT NOT FOUND]" in tool_context:
            return tool_context.replace("[APPOINTMENT NOT FOUND]", "").strip()

        if "[APPOINTMENT FOUND]" in tool_context and self.state == State.CANCEL:
            appointment_info = tool_context.replace("[APPOINTMENT FOUND]", "").strip()
            return f"{appointment_info} Please confirm if you want me to cancel this appointment."

        if "[APPOINTMENT FOUND]" in tool_context and self.state == State.RESCHEDULE:
            appointment_info = tool_context.split("[AVAILABLE SLOTS", 1)[0].replace("[APPOINTMENT FOUND]", "").strip()
            if "[AVAILABLE SLOTS" in tool_context:
                slots_block = "[AVAILABLE SLOTS" + tool_context.split("[AVAILABLE SLOTS", 1)[1]
                cleaned_slots = re.sub(r"\[AVAILABLE SLOTS.*?\]\s*", "", slots_block, count=1, flags=re.DOTALL)
                suggestions = [line.strip(" •") for line in cleaned_slots.splitlines() if line.strip()]
                if suggestions:
                    return (
                        f"{appointment_info} Here are some available replacement slots: "
                        f"{'; '.join(suggestions[:3])}. Which one should I move you to?"
                    )
            return f"{appointment_info} Tell me your preferred new date and time, and I'll help you reschedule."

        if "[BOOKING FAILED]" in tool_context:
            return "That slot is no longer available. Please choose another available date and time, and I’ll help you confirm it."

        if "[AVAILABLE SLOTS" in tool_context and self.state == State.BOOKING_DATETIME:
            cleaned = re.sub(r"\[AVAILABLE SLOTS.*?\]\s*", "", tool_context, count=1, flags=re.DOTALL)
            suggestions = [line.strip(" •") for line in cleaned.splitlines() if line.strip()]
            if suggestions:
                top_suggestions = "; ".join(suggestions[:3])
                return f"Here are some available slots: {top_suggestions}. Which one works for you?"

        if "[COST INFO]" in tool_context:
            cost_info = tool_context.replace("[COST INFO]", "").strip()
            return f"{cost_info} Would you like me to help you book an appointment for that service?"

        if "[WEATHER]" in tool_context:
            weather_info = tool_context.replace("[WEATHER]", "").strip()
            return f"{weather_info} Would you still like me to help you plan your visit?"

        prev_state = self.memory.current_state
        if "[CRM]" in tool_context and prev_state == State.GREETING:
            match = re.search(r"Returning patient on record: ([^,]+)", tool_context)
            if match:
                return f"Welcome back, {match.group(1)}. How can I help you today: booking, rescheduling, cancelling, or a clinic question?"
        ...
        if self.state == State.BOOKING_SERVICE:
            if s.service_type:
                return "Got it. I can check available appointment slots for you. Which date or time would you prefer?"
        ...
        if self.state == State.BOOKING_DATETIME:
            ...
            return "Please choose one of the available appointment slots I offered, and I will book it for you."
